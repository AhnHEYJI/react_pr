import todayView01 from "../assets/img/today/todayView01.jpg";
import todayView02 from "../assets/img/today/todayView02.jpg";
import todayView03 from "../assets/img/today/todayView03.jpg";


export const todayText = [
    {
        img: todayView01,
        title: "나만의 코딩 유튜브 사이트 만들기 프로젝트",
        desc : "webstoryboy유튜브님의 유튜브 프로젝트를 소개해드리려고 합니다.'나만의 코딩 유튜브 사이트 만들기'라는 주제로 연습해보고있습니다, 이 프로젝트의 주요 목표는 쉽게 접근하고 배우는 것입니다.아직 프로그래밍을 공부하면서 느낀 점은 양질의 많은 학습 자료와 개인의 진도에 맞는 커리큘럼을 가지고있는 유튜버 분들의 도움이 너무 감사합니다.",
        author : "webstoryboy",
        channelId: "UCsvQSv7EeCMHyYb9ENKAJZw",
        date: "2023.07.07",
        page: "/youtube"
    },
    {
        img: todayView02,
        title: "Vue.js를 이용하여 포트폴리오 사이트 만들기",
        desc : "webstoryboy유튜브님의 유튜브 프로젝트를 소개해드리려고 합니다.'나만의 코딩 유튜브 사이트 만들기'라는 주제로 연습해보고있습니다, 이 프로젝트의 주요 목표는 쉽게 접근하고 배우는 것입니다.아직 프로그래밍을 공부하면서 느낀 점은 양질의 많은 학습 자료와 개인의 진도에 맞는 커리큘럼을 가지고있는 유튜버 분들의 도움이 너무 감사합니다.",
        author : "webstoryboy",
        channelId: "UCsvQSv7EeCMHyYb9ENKAJZw",
        date: "2023.07.07",
        page: "/youtube"
    },
    {
        img: todayView03,
        title: "웹스토리보이님의 리액트강의입니다.",
        desc : "웹스토리보이 유튜브님의 유튜브 프로젝트를 소개해드리려고 합니다.React 포트폴리오 웹사이트 튜토리얼 초보자를 위한 반응형 React 프로젝트 전체 과정 유트브 사이트 입니다.",
        author : "webstoryboy",
        channelId: "7WwtzsSHdpI",
        date: "2021.04.16",
        page: "/youtube"
    }
]
